/**
 * hooks/useAlerts.ts — Lớp Toán NK v23.0
 * Smart alert engine: tính toán cảnh báo từ dữ liệu thực tế.
 */
...
export function useAlerts({
  ...
  return useMemo<SmartAlert[]>(() => {
    ...
    const smallClasses = uClasses.filter(c => {
      const n = students.filter(s => s.classId === c['Mã Lớp'] && s.status !== 'inactive').length;
      return n > 0 && n < 3;
    });
    const largeClasses = uClasses.filter(c => {
      const n = students.filter(s => s.classId === c['Mã Lớp']).length;
      return n > 20;
    });

    // Quick diary access (always show if classes exist and no diary today)
    const todayStr = new Date().toISOString().split('T')[0];
    const hasDiaryToday = tlogs.some(l => {
      const d = l.rawDate || l.date || '';
      return d.includes(todayStr.split('-')[2] + '/' + todayStr.split('-')[1] + '/' + todayStr.split('-')[0]) ||
             d.startsWith(todayStr);
    });
